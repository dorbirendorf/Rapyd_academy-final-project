...
class AccountService {
   ...
   async transferI2F(payload: ITransfer): Promise<string> {
      try {
         let { source, destination, amount } = payload;
         let sourceTransfer = (await DB_INDIVIDUAL.getAllIndividualsAccountsById([source]))[0];
         let destTransfer = await DB_FAMILY.getFamilyAccountByIdShort(destination);
         if (!sourceTransfer || !destTransfer) {
            throw new InformativeError("Data not found", "")
         }
         accountValidation.validateTransferAccounts(sourceTransfer, destTransfer, amount, 1000, "I2F");
         let ans = this.exectueTransfer(sourceTransfer.account_id, sourceTransfer.balance, destTransfer.account_id, sourceTransfer.currency, destTransfer.currency, destTransfer.balance, amount);
         return ans;
      } catch (error) {
         logger.error("transferB2BFX", error as Error);
