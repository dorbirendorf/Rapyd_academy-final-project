    ...
    constructor() {
        ...
        this.router.post(
            ...
            (req: Request, res: Response, next: NextFunction) => {
                family_validator.validateFamilyModel(req, res, next);
            },
            raw(async (req: Request, res: Response) => {
                const body = req.body as IFamilyForCreation;
                const ans = await family_service.createFamilyAccount(
                    req.accounts[0] as Partial<IFamily>,
                    body.owners,
                    body.currency
                );
        ...
        this.router.get(
            ...
            raw(async (req: Request, res: Response) => {
                const ans = await family_service.getFamilyAccountByIdFull(
        ...
        this.router.get(
            ...
            raw(async (req: Request, res: Response) => {
                const ans = await family_service.getFamilyAccountByIdShort(
                    Number(req.params.id)
                );
                const resMessage = responseFactory.createResponse(
                    ans,
        ...
        this.router.post(
            "/add/:format",
            (req: Request, res: Response, next: NextFunction) => {
                family_validator.validateUpdateAccounts(req, res, next);
            },
            raw(async (req: Request, res: Response) => {
                const body = req.body as IFamilyForCreation;
                const ans = await family_service.addIndividualsToFamilyAccount(
                    body.account_id,
                    body.owners,
                    req.params.format
                );
                const resMessage = responseFactory.createResponse(
                    ans,
                    "success add inidividuals to family",
                    201
                );
                if (req.idempotency_key) {
                    await idempotency_Db.createInstanceOfResponse(
        ...
        this.router.put(
            ...
            raw(async (req: Request, res: Response) => {
                const body = req.body as IFamilyForCreation;
                const ans =
                    await family_service.removeIndividualsFromFamilyAccount(
                        body.account_id,
                        body.owners,
                        req.params.format
                    );
                const resMessage = responseFactory.createResponse(
                    ans,
                    "remove add inidividuals from family",
                    201
                );
                if (req.idempotency_key) {
                    await idempotency_Db.createInstanceOfResponse(
