...
class FamilyService {
    async createFamilyAccount(
        family_create: Partial<IFamily>,
        owners: [number, number][],
        currency: string
    ): Promise<any> {
        try {
            logger.params("createFamilyAccount", {
                family_create,
                owners,
                currency,
            });
            ...
            let ans = await this.execAddToFamily(
                individualIds,
                accounts,
                owners,
                family,
    ...
    async execAddToFamily(
        ...
        let FamilyAccount = await this.getFamilyAccountById(
            family.account_id,
            format
        );
        if (!FamilyAccount) {
    ...
    async addIndividualsToFamilyAccount(
        ...
        try {
            ...
            if (
                !accounts ||
                accounts.length === 0 ||
                accounts.length < owners.length
            ) {
                throw new InformativeError("Data not found");
            }
            family_validator.validateAddToFamily(
                accounts,
                owners,
                family.currency
            );
            let updatedFamily = this.execAddToFamily(
                individualIds,
                accounts,
                owners,
                family,
    ...
    async getFamilyAccountByIdShort(familyId: number): Promise<IFamily> {
        try {
            ...
            let familyAccount = await DB_FAMILY.getFamilyAccountByIdShort(
                Number(familyId)
            );
            if (!familyAccount) {
                throw new InformativeError("Data not found");
    ...
    async getFamilyAccountByIdFull(familyId: number): Promise<IFamily> {
        try {
            ...
            let familyAccount = await DB_FAMILY.getFamilyAccountByIdFull(
                Number(familyId)
            );
            if (!familyAccount) {
                throw new InformativeError("Data not found");
        ...
        try {
            ...
            if (family.status === "inactive") {
                throw new InformativeError(
                    ...
                    `family ${family.account_id} accout is alreay close`
                );
            }
            if ((family.owners as IIndividual[]).length > 0) {
                throw new InformativeError(
                    config.errors.ACCOUNT_STATUS_FIELD,
                    ` family ${family.account_id} still have owners`
                );
            }
    ...
    async removeIndividualsFromFamilyAccount(
        ...
        try {
            ...
            const accounts: IIndividual[] =
                await DB_INDIVIDUAL.getAllIndividualsAccountsById(
                    utils.convertTupelsToArray(owners)
                );
            if (
                !accounts ||
                accounts.length === 0 ||
            ...
            if ((family.owners as IIndividual[]).length > owners.length) {
                //remove part of family members
                removeBalance = validation_func.sumFamilyAmounts(
                    owners,
                    family.balance - config.constants.MIN_FAMILY_BALANCE,
                    false
                ); //should be >=5000
            }

            const family_account = await this.execRemoveFromFamily(
                accounts,
                owners,
                family,
                removeBalance,
            ...
            logger.funcRet(
                "removeIndividualsFromFamilyAccount",
                family_account
            );
    ...
    async execRemoveFromFamily(
        ...
        logger.params("execRemoveFromFamily", {
            accounts,
            owners,
            family,
            removeBalance,
