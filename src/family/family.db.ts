...
export async function createFamilyAccount(family: Family): Promise<number> {
...
export async function getFamilyAccountByIdShort(familyId: string): Promise<Family> {
    const accountRes = await selectRowByIdWithJoin("account", "family", { primary_id: familyId }, "primary_id", "account_id");
    const familyAccount = addOwners_idToFamily((accountRes as RowDataPacket[])[0] as Family)
    return familyAccount;
}

async function addOwners_idToFamily(account: Family): Promise<Family> {
...
export async function getFamilyAccountByIdFull(familyId: string): Promise<Family> {
    const accountRes = await selectRowByIdWithJoin("account", "family", { primary_id: familyId }, "primary_id", "account_id");
    const familyAccount = addOwnersToFamily((accountRes as RowDataPacket[])[0] as Family)
    return familyAccount;
}

async function addOwnersToFamily(account: Family): Promise<Family> {
    account.owners = await getAllFamilyMembers(String(account.account_id));
    return account
}

export async function getAllFamilyMembers(familyId: string): Promise<Individual[]> {
    ...
    return (rows as RowDataPacket[]) as Individual[];
}
