...
class FamilyDb {
    async createFamilyAccount(family: Partial<IFamily>): Promise<number> {
        try {
            ...
            await DbHandler.createMultipleRows("family", [
                { account_id, context: family.context },
            ]);
    ...
    async getFamilyAccountByIdShort(accountId: number): Promise<IFamily> {
        try {
            ...
            const accountRes = await DbHandler.selectRowByIdWithJoin(
                "account",
                "family",
            ...
            const familyAccount = this.addOwners_idToFamily(
                parser.parseFamilyFromObj(accountRes[0] as IFamilyFromDb)
            );
    ...
    async addOwners_idToFamily(account: IFamily): Promise<IFamily> {
        try {
            ...
            account.owners_id = await this.getAllFamilyMembersId(
                account.account_id
            );
    ...
    async getAllFamilyMembersId(
        familyId: number
    ): Promise<{ account_id: number }[]> {
        try {
            logger.params("getAllFamilyMembersId", { familyId });
            const rows = await DbHandler.selectRowById(
                "family_individuals",
                { family_id: familyId },
                ["individual_id"]
            );
            const accounts_id = rows.map((obj) => {
                return { account_id: obj.individual_id };
            });
    ...
    async getFamilyAccountByIdFull(familyId: number): Promise<IFamily> {
        try {
            ...
            const familyAccount = this.addOwnersToFamily(
                parser.parseFamilyFromObj(accountRes[0] as IFamilyFromDb)
            );
    ...
    async getAllFamilyMembers(familyId: number): Promise<IIndividual[]> {
        try {
            ...
            const [rows] = (await db.query(
                `SELECT * FROM account JOIN individual on account.primary_id =  individual.account_id
    ...
    where family_individuals.family_id = ?`,
                [familyId]
            )) as RowDataPacket[][];

            const individuals = (rows as IIndividualFromDB[]).map((account) =>
                parser.parseIndividualFromObj(account)
            );
    ...
    async addIndividualsToFamilyAccount(
        ...
        try {
            logger.params("addIndividualsToFamilyAccount", {
                family_id,
                payload,
            });

            const res = await DbHandler.createMultipleRows(
                "family_individuals",
                payload.map((individual_id) => {
                    return { family_id, individual_id };
                })
            );
    ...
    async removeIndividualsFromFamilyAccount(
        familyId: number,
        payload: [number, number][]
    ): Promise<sqlRes> {
        try {
            logger.params("removeIndividualsFromFamilyAccount", {
                familyId,
                payload,
            });
            const orString = payload
                .map((pair) => "individual_id = " + String(pair[0]))
                .join(" OR ");
            const [res] = await db.query(
                `DELETE FROM family_individuals WHERE family_id = ${familyId} AND (${orString})`
            );
