...
class DbHandler {
    async createMultipleRows(
        ...
        try {
            ...
            const SingleQuestionMarksString =
                this.createQuestionMarksForInsert(parameters);
            const questionMarks = objData
                .map(() => `(${SingleQuestionMarksString})`)
                .join(",");
            const splitedValues = this.convertArrayOfArraysToMergedArray(
                objData.map((obj) => Object.values(obj) as string[])
            );

            const [rows] = await db.query(
                `INSERT INTO ${tableName} (${parameters})  VALUES ${questionMarks}`,
                splitedValues
            );
    ...
    async updaetRowById(
        ...
        try {
            ...
            const whereString = this.createWhereString(
                Object.keys(objId)[0],
    ...
    async updateMultipleRowsById(
        tableName: string,
        objData: object[],
        objId: object[]
    ): Promise<sqlRes> {
        try {
            logger.params("updateMultipleRowsById", {
                tableName,
                objId,
                objData,
            });
            ...
            const [rows] = await db.query(
                `UPDATE ${tableName} SET ${data_name} = CASE ${whenStrings} END WHERE ${orString}`
            );
    ...
    async deleteRowById(tableName: string, objId: object): Promise<sqlRes> {
        try {
            ...
            const [rows] = await db.query(
                `DELETE FROM ${tableName} WHERE ${whereString}`
            );
    ...
    async selectRowByIdWithJoin(
        ...
        try {
            logger.params("selectRowByIdWithJoin", {
                firstTableName,
                secondTableName,
                columnNames,
                onFirst,
            ...
            const columnString = this.createColumnStringForSelect(columnNames);
            const whereString = this.createWhereString(
                `${firstTableName}.${Object.keys(objId)[0]}`,
                String(Object.values(objId)[0])
            );
            const onString = `${firstTableName}.${onFirst}=${secondTableName}.${onSecond}`;
            const [rows] = await db.query(
                `SELECT ${columnString} FROM ${firstTableName} JOIN ${secondTableName} ON ${onString} WHERE ${whereString}`
            );
    ...
    async selectAllRow(
        ...
        try {
            ...
            const columnString = this.createColumnStringForSelect(columnNames);
            const [rows] = await db.query(
                `SELECT ${columnString} FROM ${tableName}`,
    ...
    private createWhenThenString(objData: object[], objId: object[]) {
        try {
            ...
            for (let i = 0; i < objData.length; i++) {
                let line: string = ` WHEN ${
                    Object.keys(objId[i])[0]
                } = ${String(Object.values(objId[i])[0])} THEN ${String(
                    Object.values(objData[i])[0]
                )}`;
    ...
    private createSetString(objData: object) {
        try {
            ...
            const setString = Object.entries(objData)
    ...
    private createWhereStringWithOrBetween(objId: object[]) {
        try {
            ...
            const whereString = objId
                .map((obj) => Object.entries(obj)[0])
                .map((pair) => this.createWhereString(pair[0], String(pair[1])))
                .join(" OR ");
    ...
    private createQuestionMarksForInsert(parameters: string) {
        try {
            ...
            const questionMarks = parameters
                .split(",")
                .map(() => "?")
                .join(",");
