...
class ValidationFunctions {
    ...
    sumFamilyAmounts(
        ...
        try {
            ...
            const sum: number = tupels.reduce(
                (prev, tupel) => tupel[1] + prev,
                0
            );
            if (min) {
                if (sum < minBalance) {
                    throw new InformativeError(
                        ...
                        `sum of all amounts should be ${minBalance}`
                    );
                }
            } else {
                if (sum > minBalance) {
                    throw new InformativeError(
                        config.errors.ACCOUNT_BALLANCE_LOW,
                        ` sum of all amounts can't be more then ${minBalance}`
                    );
                }
            }
    ...
    initRequiredParams(): Map<string, string[]> {
        try {
            ...
            requiredParams.set("individual", [
                "individual_id",
                "first_name",
                "last_name",
                "currency",
            ]);
            requiredParams.set("business", [
                "compay_id",
                "company_name",
                "currency",
            ]);
