    ...
    constructor() {
        ...
        this.router.post(
            ...
            (req: Request, res: Response, next: NextFunction) => {
                individual_validator.validateIndividualModel(req, res, next);
            },
            raw(async (req: Request, res: Response) => {
                const id = await individual_service.createIndividualAccount(
                    req.accounts[0] as Partial<IIndividual>
                );
                const ans = await individual_service.getIndividualByAccountId(
                    String(id)
                );
