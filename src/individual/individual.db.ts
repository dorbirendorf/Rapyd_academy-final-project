...
class IndividualDb {
    async createIndividualAccount(
        ...
        try {
            ...
            const address_id = await account_db.createAddress(
                individual.address
            );
            await DbHandler.createMultipleRows("individual", [
                {
                    account_id,
    ...
    async getAllIndividualsAccountsById(
        accounts_id: number[]
    ): Promise<IIndividual[]> {
        try {
            ...
            const orString = accounts_id
                ...
                .join(" OR ");
            const [rows] =
                (await db.query(`SELECT * FROM individual JOIN account on individual.account_id = account.primary_id
   LEFT JOIN address on address.address_id = individual.address_id WHERE ${orString}`)) as RowDataPacket[][];
    ...
    async checkIfIndivdualExistByIndividualId(
        ...
        try {
            logger.params("checkIfIndivdualExistByIndividualId", {
                individual_id,
            });

            const rows = await DbHandler.selectRowById("individual", {
                individual_id,
            });
