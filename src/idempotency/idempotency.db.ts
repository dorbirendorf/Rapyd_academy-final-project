...
class IdempotencyDb
{ async  createInstanceOfResponse(message:httpResponseMessage,idempotency_key:string,agent_id:number): Promise<void> {
   try {
      logger.params("createIdempotencyResponseAccount", { message,idempotency_key ,agent_id});
      await DbHandler.createMultipleRows("idempotency", [{ message:message.message,data:JSON.stringify(message.data),status:message.status,agent_id,idempotency_key}])
 ...
 async  getResponseFromDb(agentId:number,requestKey:string): Promise<httpResponseMessage|undefined>{
   try {
      logger.params("getResponseFromDb", { agentId,requestKey });
     
      const [rows] = (await db.query(`SELECT status,message,data FROM idempotency WHERE agent_id = ${agentId} AND idempotency_key = "${requestKey}"`)) as RowDataPacket[][]
      if(!rows[0]){
